...
const questoesD1 = [
  { numero: 1, codigo: 'q1', descricao: 'Lê números (mostrar 2.457)', tipo: 'oral' },
  { numero: 2, codigo: 'q2', descricao: 'Escreve números (falar "mil duzentos e trinta e quatro")', tipo: 'escrito' },
  { numero: 3, codigo: 'q3', descricao: 'Lê frase curta', tipo: 'oral' },
  { numero: 4, codigo: 'q4', descricao: 'Interpreta comando ("Circule o maior número")', tipo: 'escrito' },
  { numero: 5, codigo: 'q5', descricao: 'Sequência lógica (figuras)', tipo: 'visual' },
  { numero: 6, codigo: 'q6', descricao: 'Copia informação corretamente', tipo: 'escrito' },
]

export default function LancarD1Page() {
  ...
  function calcularGrupo(respostas: Aluno['respostas']): string {
  ...
  async function salvarLancamento() {
    ...
    try {
      ...
      if (!diagD1) throw new Error('Diagnóstico D1 não encontrado')
      ...
      for (const aluno of alunos) {
        ...
        if (aluno.presente && questoes) {
          for (const questao of questoes) {
            const respostaKey = `q${questao.numero}` as keyof typeof aluno.respostas
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">Lançar D1 - Habilidades Fundamentais</h1>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {questoesD1.map((q) => (
                    <th key={q.codigo} className="px-3 py-3 text-center text-sm font-medium text-gray-700 w-24">
                      Q{q.numero}
                    </th>
                  ))}
                  <th className="px-3 py-3 text-center text-sm font-medium text-gray-700 w-20">
                ...
                {alunos.map((aluno) => {
                  ...
                  return (
                    <tr key={aluno.id} className={!aluno.presente ? 'bg-gray-50 opacity-60' : ''}>
                      ...
                      {questoesD1.map((q) => (
                        <td key={q.codigo} className="px-3 py-3 text-center">
                          {aluno.presente ? (
                            <select
                              value={aluno.respostas[q.codigo as keyof typeof aluno.respostas]}
                              onChange={(e) => atualizarResposta(aluno.id, q.codigo, e.target.value)}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          {alunoAtual.presente && (
            <div className="space-y-4">
              {questoesD1.map((q) => (
                <div key={q.codigo} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="font-medium text-gray-900">Questão {q.numero}</p>
                      <p className="text-sm text-gray-500">{q.descricao}</p>
                    ...
                    <div className="flex gap-2">
                      {['acertou', 'parcial', 'errou'].map((valor) => (
                        <button
                          key={valor}
                          onClick={() => atualizarResposta(alunoAtual.id, q.codigo, valor)}
                          className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                            alunoAtual.respostas[q.codigo as keyof typeof alunoAtual.respostas] === valor
              ...
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-600">Grupo calculado:</p>
                <p className={`text-2xl font-bold ${
                  calcularGrupo(alunoAtual.respostas) === 'A' ? 'text-red-600' :
                  calcularGrupo(alunoAtual.respostas) === 'B' ? 'text-yellow-600' :
                  calcularGrupo(alunoAtual.respostas) === 'C' ? 'text-green-600' :
                  'text-gray-400'
                }`}>
                  Grupo {calcularGrupo(alunoAtual.respostas)}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-center gap-3 p-3 bg-green-50 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-green-600">Acertou 5-6 questões</p>
            </div>
          </div>
        </div>
      </div>
