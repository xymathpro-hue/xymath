...
export default function SimuladosPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [sRes, tRes, qRes, hRes, dRes] = await Promise.all([
        ...
        supabase.from('questoes').select('id, enunciado, ano_serie, dificuldade, habilidade_bncc_id, is_publica, ativa, alternativa_a, alternativa_b, alternativa_c, alternativa_d, alternativa_e, resposta_correta, fonte, descritor_codigo, unidade_tematica').eq('ativa', true),
  ...
  const handleGoToPreview = () => {
    if (!formData.titulo.trim()) { setSaveError('Digite um título'); return }
    if (questoesSelecionadas.length === 0) { setSaveError('Selecione pelo menos uma questão'); return }
    setSaveError(null)
    setModalStep('preview')
  }

  const handleBackToForm = () => { setSaveError(null); setModalStep('form') }
  ...
  const gerarQuestoesAutomaticamente = () => {
    let qFiltradas = [...questoesDisponiveis]
    
    // Se NÃO for TEMA, filtrar por ano
    ...
    if (autoConfig.habilidades_ids.length > 0) {
      ...
      } else if (autoConfig.fonte === 'TEMA') {
        qFiltradas = qFiltradas.filter(q => q.unidade_tematica && autoConfig.habilidades_ids.includes(q.unidade_tematica))
      } else {
  ...
  const handleExport = async (simulado: Simulado, formato: 'word' | 'pdf') => {
    setExportMenuOpen(null)
    try {
      const { data: questoesData } = await supabase.from('questoes').select('id, enunciado, alternativa_a, alternativa_b, alternativa_c, alternativa_d, alternativa_e, resposta_correta, dificuldade, habilidade_bncc_id, descritor_codigo').in('id', simulado.questoes_ids)
      if (!questoesData) return
      const questoesOrdenadas = simulado.questoes_ids.map(id => questoesData.find(q => q.id === id)).filter(Boolean).map(q => ({ ...q!, habilidade_codigo: habilidades.find(h => h.id === q!.habilidade_bncc_id)?.codigo || q!.descritor_codigo }))
      const turmaNome = getTurmasNomes(simulado)
      const config = { titulo: simulado.titulo, subtitulo: simulado.descricao, turma: turmaNome, tempo: simulado.tempo_minutos, incluirGabarito: true, incluirCabecalho: true, questoes: questoesOrdenadas }
      if (formato === 'word') await exportToWord(config); else await exportToPDF(config)
  ...
  const filteredQuestoes = questoesDisponiveis.filter(q => {
    ...
    if (questaoFilters.habilidades_ids.length > 0) {
      ...
      } else if (questaoFilters.fonte === 'TEMA') {
        if (!q.unidade_tematica || !questaoFilters.habilidades_ids.includes(q.unidade_tematica)) return false
      } else {
  ...
  const habilidadesFiltradas = habilidades.filter(h => !autoConfig.ano_serie || h.codigo.includes(autoConfig.ano_serie.charAt(0)))
  const habilidadesFiltradasManual = habilidades.filter(h => !questaoFilters.ano_serie || h.codigo.includes(questaoFilters.ano_serie.charAt(0)))
  ...
  const getQuestoesDisponiveisPorDificuldade = () => {
    ...
    if (autoConfig.fonte !== 'TEMA') {
      qf = qf.filter(q => q.ano_serie === autoConfig.ano_serie)
    }
    ...
    if (autoConfig.habilidades_ids.length > 0) {
      if (autoConfig.fonte === 'SAEB') {
        ...
        qf = qf.filter(q => q.unidade_tematica && autoConfig.habilidades_ids.includes(q.unidade_tematica))
      } else {
  ...
  const getEstatisticasSimulado = () => {
    ...
    return {
      ...
      habilidades: [...new Set(questoes.map(q => q.habilidade_bncc_id || q.descritor_codigo || q.unidade_tematica).filter(Boolean))].length
    }
  }
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2 flex-wrap"><h3 className="font-semibold text-gray-900">{s.titulo}</h3><Badge variant={getStatusVariant(s.status)}>{s.status}</Badge></div>
                    ...
                    <div className="relative" ref={exportMenuRef}>
                      ...
                      {exportMenuOpen === s.id && (
                        <div className="absolute right-0 top-full mt-1 bg-white border rounded-lg shadow-lg z-10 min-w-[140px]">
                          <button onClick={() => handleExport(s, 'word')} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 flex items-center gap-2"><FileDown className="w-4 h-4 text-blue-600" />Word</button>
                          <button onClick={() => handleExport(s, 'pdf')} className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 flex items-center gap-2"><FileDown className="w-4 h-4 text-red-600" />PDF</button>
                        </div>
                      )}
                    </div>
        ...
        {modalStep === 'form' ? (
            ...
            <div>
              ...
              {showTurmasSelector && (
                <div className="mt-2 border rounded-lg p-3 bg-gray-50 max-h-40 overflow-y-auto">
                  {turmas.length === 0 ? <p className="text-sm text-gray-500 text-center py-2">Nenhuma turma</p> : turmas.map(turma => (
          ...
          <div className="space-y-4">
            <div className="bg-indigo-50 p-4 rounded-lg">
              ...
              <div className="flex gap-3 mt-2 text-xs"><span>🟢 {stats.facil}</span><span>🟡 {stats.medio}</span><span>🔴 {stats.dificil}</span></div>
            </div>
            <div>
            ...
            {saveError && <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-start gap-2"><AlertCircle className="w-5 h-5 text-red-500 flex-shrink-0" /><p className="text-sm text-red-600">{saveError}</p></div>}
            <div className="flex gap-3 pt-4 border-t"><Button variant="outline" onClick={handleBackToForm}><ArrowLeft className="w-4 h-4 mr-1" />Voltar</Button><Button className="flex-1" onClick={handleSave} loading={saving}><CheckCircle className="w-4 h-4 mr-2" />{editingSimulado ? 'Salvar' : 'Criar'}</Button></div>
          </div>
        )}
      </Modal>

      {/* MODAL SELECIONAR QUESTÕES */}
      <Modal isOpen={questoesModalOpen} onClose={() => setQuestoesModalOpen(false)} title="Selecionar Questões" size="xl">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            <select className="px-3 py-2 border rounded-lg text-gray-900" value={questaoFilters.ano_serie} onChange={(e) => setQuestaoFilters({ ...questaoFilters, ano_serie: e.target.value })} disabled={questaoFilters.fonte === 'TEMA'}>
              <option value="">{questaoFilters.fonte === 'TEMA' ? 'Todos os anos' : 'Todos os anos'}</option>
            ...
            <select className="px-3 py-2 border rounded-lg text-gray-900" value={questaoFilters.fonte} onChange={(e) => setQuestaoFilters({ ...questaoFilters, fonte: e.target.value, habilidades_ids: [], ano_serie: e.target.value === 'TEMA' ? '' : questaoFilters.ano_serie })}>
              ...
              {questaoFilters.fonte === 'SAEB' ? (
                descritoresSaeb.map(d => (
                  <label key={d.codigo} className="flex items-center gap-2 p-1 hover:bg-white rounded cursor-pointer">
                    <input type="checkbox" checked={questaoFilters.habilidades_ids.includes(d.codigo)} onChange={() => toggleHabilidadeManual(d.codigo)} className="rounded" />
                    <span className="text-sm text-gray-900"><strong>{d.codigo}</strong> - {d.descricao.substring(0, 40)}...</span>
                  </label>
                ))
              ) : questaoFilters.fonte === 'TEMA' ? (
                temasDisponiveis.length > 0 ? temasDisponiveis.map(tema => (
                  <label key={tema} className="flex items-center gap-2 p-1 hover:bg-white rounded cursor-pointer">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select className="w-full px-3 py-2 border rounded-lg text-gray-900" value={autoConfig.ano_serie} onChange={(e) => setAutoConfig({ ...autoConfig, ano_serie: e.target.value, habilidades_ids: [] })} disabled={autoConfig.fonte === 'TEMA'}>
                  {ANO_SERIE_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                </select>
              </div>
              <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {autoConfig.fonte === 'SAEB' ? 'Descritores' : autoConfig.fonte === 'TEMA' ? 'Temas' : 'Habilidades'} (opcional)
              </label>
              <div className="max-h-28 overflow-y-auto border rounded-lg p-2">
                ...
                ) : autoConfig.fonte === 'TEMA' ? (
                  temasDisponiveis.length > 0 ? temasDisponiveis.map(tema => (
                    <label key={tema} className="flex items-center gap-2 p-1 hover:bg-gray-50 rounded cursor-pointer">
