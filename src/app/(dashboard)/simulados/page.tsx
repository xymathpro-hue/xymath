...
import { Plus, Search, FileText, Edit, Trash2, Eye, Play, QrCode, Check, X } from 'lucide-react'
...
export default function SimuladosPage() {
  ...
  const [questaoFilters, setQuestaoFilters] = useState({ ano_serie: '', unidade_tematica: '', dificuldade: '' })
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div><h1 className="text-2xl font-bold">Simulados</h1><p className="text-gray-600">Crie e gerencie simulados</p></div>
        <Link href="/simulados/novo"><Button><Plus className="w-5 h-5 mr-2" />Novo Simulado</Button></Link>
      </div>
      ...
      <Card variant="bordered" className="mb-6"><CardContent className="p-4">
        ...
        <Input placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" /></div>
      </CardContent></Card>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card variant="bordered"><CardContent className="p-4"><p className="text-sm text-gray-600">Total</p><p className="text-2xl font-bold">{simulados.length}</p></CardContent></Card>
        ...
        <Card variant="bordered"><CardContent className="p-4"><p className="text-sm text-gray-600">Encerrados</p><p className="text-2xl font-bold">{simulados.filter(s => s.status === 'encerrado').length}</p></CardContent></Card>
      </div>
      ...
      : filteredSimulados.length === 0 ? (
        <Card variant="bordered"><CardContent className="p-12 text-center">
          ...
          <h3 className="text-lg font-medium mb-2">{searchTerm ? 'Nenhum encontrado' : 'Nenhum simulado'}</h3>
          ...
          <Card key={s.id} variant="bordered" className="hover:shadow-md transition-shadow"><CardContent className="p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2"><h3 className="font-semibold">{s.titulo}</h3><Badge variant={getStatusVariant(s.status)}>{s.status}</Badge></div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingSimulado ? 'Editar' : 'Novo Simulado'} size="xl">
        <div className="space-y-4">
          ...
          <textarea className="w-full px-4 py-2 border rounded-lg" rows={2} value={formData.descricao} onChange={(e) => setFormData({ ...formData, descricao: e.target.value })} /></div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input label="Tempo (min)" type="number" value={formData.tempo_minutos} onChange={(e) => setFormData({ ...formData, tempo_minutos: parseInt(e.target.value) || 60 })} />
          </div>
          <div className="flex gap-4">
            <label className="flex items-center gap-2"><input type="checkbox" checked={formData.embaralhar_questoes} onChange={(e) => setFormData({ ...formData, embaralhar_questoes: e.target.checked })} className="rounded" /><span className="text-sm">Embaralhar questões</span></label>
            <label className="flex items-center gap-2"><input type="checkbox" checked={formData.embaralhar_alternativas} onChange={(e) => setFormData({ ...formData, embaralhar_alternativas: e.target.checked })} className="rounded" /><span className="text-sm">Embaralhar alternativas</span></label>
          </div>
          <div className="border-t pt-4">
            <div className="flex items-center justify-between mb-4"><h4 className="font-medium">Questões: {questoesSelecionadas.length}</h4>
            <Button variant="outline" size="sm" onClick={() => setQuestoesModalOpen(true)}><Plus className="w-4 h-4 mr-1" />Adicionar</Button></div>
            {questoesSelecionadas.length > 0 && <div className="space-y-2 max-h-40 overflow-y-auto">
              {questoesSelecionadas.map((id, idx) => { const q = questoesDisponiveis.find(x => x.id === id); return q ? (
                <div key={id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                  <span className="text-sm"><strong>{idx + 1}.</strong> {q.enunciado.substring(0, 50)}...</span>
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <Button className="flex-1" onClick={handleSave} loading={saving} disabled={!formData.titulo || questoesSelecionadas.length === 0}>{editingSimulado ? 'Salvar' : 'Criar'}</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={questoesModalOpen} onClose={() => setQuestoesModalOpen(false)} title="Selecionar Questões" size="xl">
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <Select options={ANO_SERIE_OPTIONS} placeholder="Ano" value={questaoFilters.ano_serie} onChange={(e) => setQuestaoFilters({ ...questaoFilters, ano_serie: e.target.value })} />
            <Select options={UNIDADES_TEMATICAS} placeholder="Unidade" value={questaoFilters.unidade_tematica} onChange={(e) => setQuestaoFilters({ ...questaoFilters, unidade_tematica: e.target.value })} />
            <Select options={DIFICULDADE_OPTIONS} placeholder="Dificuldade" value={questaoFilters.dificuldade} onChange={(e) => setQuestaoFilters({ ...questaoFilters, dificuldade: e.target.value })} />
          </div>
            ...
            {filteredQuestoes.map(q => { const sel = questoesSelecionadas.includes(q.id); return (
              <div key={q.id} className={`p-3 border rounded-lg cursor-pointer ${sel ? 'bg-indigo-50 border-indigo-300' : 'hover:bg-gray-50'}`} onClick={() => toggleQuestao(q.id)}>
                <div className="flex items-start gap-3">
                  <div className={`w-5 h-5 rounded border flex items-center justify-center ${sel ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300'}`}>{sel && <Check className="w-3 h-3 text-white" />}</div>
                  <div className="flex-1"><div className="flex gap-2 mb-1"><Badge variant="info">{q.ano_serie}</Badge><Badge>{q.dificuldade}</Badge></div><p className="text-sm line-clamp-2">{q.enunciado}</p></div>
                </div>
              </div>
            )})}
          </div>
          <div className="flex gap-3 pt-4 border-t"><Button variant="outline" className="flex-1" onClick={() => setQuestoesModalOpen(false)}>Fechar</Button><p className="flex-1 text-center py-2">{questoesSelecionadas.length} selecionadas</p></div>
        </div>
      </Modal>
    </div>
  )
