...
import { Plus, Search, FileText, Edit, Trash2, Eye, Play, QrCode, Check, X, Wand2, Filter } from 'lucide-react'
    ...
    <div className="p-6 lg:p-8">
                ...
                {questoesSelecionadas.map((id, idx) => {
                  ...
                  return q ? (
                    <div key={id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      <span className="text-sm text-gray-900"><strong>{idx + 1}.</strong> {q.enunciado.substring(0, 50)}...
      ...
      <Modal isOpen={gerarAutoModalOpen} onClose={() => setGerarAutoModalOpen(false)} title="Gerar Simulado Automático" size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Questões Fáceis</label><Input type="number" min={0} value={autoConfig.qtd_facil} onChange={(e) => setAutoConfig({ ...autoConfig, qtd_facil: parseInt(e.target.value) || 0 })} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Questões Médias</label><Input type="number" min={0} value={autoConfig.qtd_medio} onChange={(e) => setAutoConfig({ ...autoConfig, qtd_medio: parseInt(e.target.value) || 0 })} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Questões Difíceis</label><Input type="number" min={0} value={autoConfig.qtd_dificil} onChange={(e) => setAutoConfig({ ...autoConfig, qtd_dificil: parseInt(e.target.value) || 0 })} /></div>
          </div>
          <div className="bg-blue-50 p-3 rounded-lg"><p className="text-sm text-blue-800"><strong>Total:</strong> {autoConfig.qtd_facil + autoConfig.qtd_medio + autoConfig.qtd_dificil} questões</p></div>
          <div className="flex gap-3 pt-4 border-t">
