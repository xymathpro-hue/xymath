...
export default function SimuladosPage() {
  ...
  const handleOpenModal = (s?: Simulado) => {
    if (s) {
      ...
      setFormData({
        ...
        turmas_ids: s.turma_id ? [s.turma_id] : (s.turmas_ids || []),
  ...
  const getTurmasNomes = (s: Simulado) => {
    const ids = s.turmas_ids || (s.turma_id ? [s.turma_id] : [])
    return ids.map(id => turmas.find(t => t.id === id)?.nome).filter(Boolean).join(', ')
  }
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-1">
                    {s.status === 'rascunho' && (
                      <Button variant="ghost" size="sm" onClick={() => handlePublish(s.id)}>
        ...
        <div className="space-y-4">
                ...
                {formData.turmas_ids.map(id => {
                  ...
                  return turma ? (
                    <Badge key={id} variant="info" className="flex items-center gap-1">
                      {turma.nome}
                      <button onClick={() => toggleTurma(id)} className="ml-1 hover:text-red-500">
                        <X className="w-3 h-3" />
                      </button>
                    </Badge>
                  ) : null
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <Input 
                ...
                onChange={(e) => setFormData({ ...formData, total_questoes: parseInt(e.target.value) || 10 })} 
              />
            </div>
            <div>
            ...
            <div className="flex flex-col justify-end">
              <p className="text-sm text-gray-500">
                ~{Math.round(formData.tempo_minutos / formData.total_questoes)} min/questão
              </p>
            </div>
          </div>
          ...
          <div className="border-t pt-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {questoesSelecionadas.length} de {formData.total_questoes} questões
                  {questoesSelecionadas.length < formData.total_questoes && (
                    <span className="text-orange-500 ml-2">
                      (faltam {formData.total_questoes - questoesSelecionadas.length})
                    </span>
                  )}
                </p>
              </div>
              <div className="flex gap-2">
                ...
                <Button variant="outline" size="sm" onClick={() => setQuestoesModalOpen(true)}>
                  <Plus className="w-4 h-4 mr-1" />Selecionar Manual
                </Button>
              </div>
            </div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 mb-4">
              <div 
                ...
                style={{ width: `${Math.min((questoesSelecionadas.length / formData.total_questoes) * 100, 100)}%` }}
              />
            </div>
                  ...
                  return q ? (
                    <div key={id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      <span className="text-sm text-gray-900">
                        <strong>{idx + 1}.</strong> {q.enunciado.substring(0, 60)}...
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <Button 
              ...
              onClick={handleSave} 
              loading={saving} 
              disabled={!formData.titulo || questoesSelecionadas.length === 0}
            >
              {editingSimulado ? 'Salvar' : 'Criar Simulado'}
            </Button>
          </div>
        </div>
      </Modal>

      {/* Modal Seleção Manual - COM MÚLTIPLAS HABILIDADES */}
        ...
        <div className="space-y-4">
          <div className="bg-indigo-50 p-3 rounded-lg flex items-center justify-between">
            <span className="text-sm text-indigo-700">
              <strong>{questoesSelecionadas.length}</strong> de <strong>{formData.total_questoes}</strong> questões selecionadas
            </span>
            {questoesSelecionadas.length >= formData.total_questoes && (
            ...
            <div className="border rounded-lg p-3 bg-gray-50">
              <div className="flex items-center justify-between mb-2">
                ...
                {questaoFilters.habilidades_ids.length > 0 && (
                  <Button variant="ghost" size="sm" onClick={() => setQuestaoFilters({ ...questaoFilters, habilidades_ids: [] })}>Limpar</Button>
                )}
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {questaoFilters.habilidades_ids.map(id => {
                ...
                return h ? (
                  <Badge key={id} variant="info" className="flex items-center gap-1">
                    {h.codigo}
                    <button onClick={() => toggleHabilidadeManual(id)} className="ml-1 hover:text-red-500"><X className="w-3 h-3" /></button>
                  </Badge>
                ) : null
              })}
            </div>
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <div className="flex-1 text-center py-2">
              <span className="font-medium text-gray-900">{questoesSelecionadas.length}/{formData.total_questoes}</span>
              <span className="text-gray-500 ml-1">selecionadas</span>
            </div>
          </div>
        </div>
      </Modal>
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 p-3 rounded-lg">
            <p className="text-sm text-blue-800">
              Meta: <strong>{formData.total_questoes} questões</strong> (definido na tela anterior)
            </p>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <Input type="number" min={0} value={autoConfig.qtd_facil} onChange={(e) => setAutoConfig({ ...autoConfig, qtd_facil: parseInt(e.target.value) || 0 })} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">🟡 Médias</label>
                <Input type="number" min={0} value={autoConfig.qtd_medio} onChange={(e) => setAutoConfig({ ...autoConfig, qtd_medio: parseInt(e.target.value) || 0 })} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">🔴 Difíceis</label>
                <Input type="number" min={0} value={autoConfig.qtd_dificil} onChange={(e) => setAutoConfig({ ...autoConfig, qtd_dificil: parseInt(e.target.value) || 0 })} />
              </div>
            </div>
          </div>

          <div className={`p-3 rounded-lg ${
            autoConfig.qtd_facil + autoConfig.qtd_medio + autoConfig.qtd_dificil === formData.total_questoes
              ? 'bg-green-50'
              : 'bg-orange-50'
          }`}>
            <p className={`text-sm ${
              ...
              {autoConfig.qtd_facil + autoConfig.qtd_medio + autoConfig.qtd_dificil !== formData.total_questoes && (
                <span className="ml-2">(meta: {formData.total_questoes})</span>
              )}
            </p>
          </div>
