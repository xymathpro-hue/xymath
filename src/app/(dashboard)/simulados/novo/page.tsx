...
export default function NovoSimuladoPage() {
  ...
  // Buscar questões do banco com filtros - CORRIGIDO
  const buscarQuestoes = useCallback(async () => {
    ...
    try {
      ...
      // CORRIGIDO: habilidade_bncc_id
      if (filters.habilidades_ids.length > 0) {
        query = query.in('habilidade_bncc_id', filters.habilidades_ids)
      }
      // CORRIGIDO: descritor_saeb_id
      if (filters.descritores_ids.length > 0) {
        query = query.in('descritor_saeb_id', filters.descritores_ids)
      }
  ...
  // Helpers - CORRIGIDO para usar habilidade_bncc_id
  ...
  // Salvar simulado - CORRIGIDO com valor_total e pontuacao_questao calculada
  const handleSave = async (status: 'rascunho' | 'publicado') => {
    ...
    try {
      ...
      // Calcular pontuação por questão baseado no valor total
      const pontuacaoQuestao = formData.valor_total / questoesSelecionadas.length
      ...
      const simuladoData = {
        ...
        configuracoes: {
          ...
          pontuacao_questao: pontuacaoQuestao,
  ...
  // CORRIGIDO: Calcular valor por questão baseado no valor total
  const pontuacaoQuestao = questoesSelecionadas.length > 0 
    ? formData.valor_total / questoesSelecionadas.length 
    : 0
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Valor Total do Simulado (pontos)
                </label>
                <input
                  type="number"
                  step="0.5"
                  ...
                  onChange={(e) => setFormData({ ...formData, valor_total: parseFloat(e.target.value) || 10 })}
                />
                <p className="text-xs text-gray-500 mt-1">
                  O valor de cada questão será calculado automaticamente
                </p>
              </div>
            </div>
              ...
              <Card variant="bordered" className="sticky top-6">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-indigo-600 font-semibold">{formData.valor_total} pts</span>
                  </div>
                  
                  {questoesSelecionadas.length > 0 && (
                    <p className="text-xs text-gray-500 mb-4">
                      Cada questão vale: <strong>{pontuacaoQuestao.toFixed(2)} pts</strong>
                    </p>
                  )}
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{formData.valor_total} pontos</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Valor por Questão</p>
                    <p className="font-medium text-gray-900">{pontuacaoQuestao.toFixed(2)} pts</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-indigo-50 rounded-xl p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-indigo-700">Valor total:</span>
                    <span className="font-semibold text-indigo-900">{formData.valor_total} pontos</span>
                  </div>
