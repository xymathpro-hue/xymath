...
export async function gerarFolhasRespostas(dados: DadosFolha): Promise<void> {
  ...
  for (let i = 0; i < alunos.length; i++) {
    ...
    const matricula = `${turma.nome}-${new Date().getFullYear()}-${String(i + 1).padStart(3, '0')}`

    // ===============================
    // MARCADORES DE CANTO (8x8 mm)
    // ===============================
    ...
    // 🔴 MUITO IMPORTANTE: voltar para branco
    ...
    pdf.setFontSize(9)
    pdf.setFont('helvetica', 'normal')
    pdf.text(simulado.titulo, pageWidth / 2, offsetY + 41, { align: 'center' })
